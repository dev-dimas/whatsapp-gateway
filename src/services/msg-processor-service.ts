...
export const Process = async (
  m: WAMessage | undefined
): Promise<{
  needReply: boolean;
  message: AnyMessageContent | string | null;
}> => {
  if (m == undefined) {
    return {
      needReply: false,
  ...
  const formattedJid = FormatToPhoneNumber(senderJid);
  const senderName = m.pushName;
  const text = m.message?.ephemeralMessage?.message?.extendedTextMessage?.text ?? m.message?.extendedTextMessage?.text ?? m.message?.conversation;
  ...
  if (type == 'otp') {
    ...
    const validationResult = await otpService.Confirm(ref, formattedJid);
